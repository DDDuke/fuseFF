...
// 			code |= (src.at<_Tp>(i+1,j) > center) << 10;
// 			code |= (src.at<_Tp>(i+1,j-1) > center) << 9;
// 			code |= (src.at<_Tp>(i,j-1) > center) << 8;
...
void lbp::ELBP_(const Mat& src, Mat& dst, int radius, int neighbors) {
	neighbors = max(min(neighbors,maxNeighbours),1); // set bounds...
...
// 				dst.at<unsigned int>(i-radius,j-radius) -= (abs(t-src.at<_Tp>(i,j)) > std::numeric_limits<float>::epsilon()) << n;
...
void lbp::VARLBP_(const Mat& src, Mat& dst, int radius, int neighbors) {
	max(min(neighbors,maxNeighbours),1); // set bounds
	...
	Mat _std = Mat::zeros(src.rows, src.cols, CV_32FC1);
	for(int n=0; n<neighbors; n++) {
		// sample points
		float x = static_cast<float>(radius) * cos(2.0*M_PI*n/static_cast<float>(neighbors));
		float y = static_cast<float>(radius) * -sin(2.0*M_PI*n/static_cast<float>(neighbors));
		...
		for(int i=radius; i < src.rows-radius;i++) {
			for(int j=radius;j < src.cols-radius;j++) {
				...
				_std.at<float>(i,j) = sqrt(_m2.at<float>(i,j));
				dst.at<float>(i-radius,j-radius) += (_m2.at<float>(i,j) / (1.0 * neighbors));
// 				dst.at<float>(i-radius,j-radius) = ((_m2.at<float>(i,j) > src.at<_Tp>(i,j)) && (abs(_m2.at<float>(i,j)-src.at<_Tp>(i,j)) > std::numeric_limits<float>::epsilon())) << n;
			}
		}
	}
// 	// calculate result
